...
// Handler returns the home page with the requested computation
...
func processRequest(request *http.Request) ([2][2]int, string, bool) {
	...
	for matID, matName := range []string{"matASize", "matBSize"} {
		...
		if found && len(userInputString) > 0 {
			...
			for idx, value := range sizeValues {
				if intValue, err := strconv.ParseInt(value, 0, 0); err != nil {
					return matSizes, value + " is an invalid input", false
				} else {
					matSizes[matID][idx] = int(intValue)
				}
			}
		} else {
